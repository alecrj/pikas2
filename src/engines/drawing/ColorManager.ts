...
import { 
  ...
  export class ColorManager {
    ...
    private readonly HARMONY_RULES: Record<ColorHarmony, (hue: number) => number[]> = {
      ...
      'split-complementary': (h) => [
        (h + 150) % 360,
        (h + 210) % 360,
      ],
      'double-complementary': (h) => [
        (h + 30) % 360,
        (h + 180) % 360,
        (h + 210) % 360,
      ],
      ...
      'monochromatic': (h) => [h, h, h], // Different saturations/brightnesses
    };
    ...
    public removeColorFromPalette(paletteId: string, index: number): boolean {
      const palette = this.palettes.get(paletteId);
      if (!palette || index < 0 || index >= palette.colors.length) return false;
